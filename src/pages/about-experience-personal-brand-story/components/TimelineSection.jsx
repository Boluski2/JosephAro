          ...
          <div className="relative">
            ...
            <div className="space-y-8 md:space-y-12">
              {timelineData?.map((item, index) => (
                <div
                  key={index}
                  className={`relative flex flex-col items-start cursor-pointer transition-brand`}
                ...
                >
                  ...
                  {/* Content Card - Centered on mobile with full width */}
                    ...
                    <div className={`card-brand p-4 md:p-6 hover:shadow-brand-lg transition-brand ${
                      ...
                      <div className="flex items-start space-x-3 md:space-x-4">
                        <div className="flex-shrink-0">
                          <div className="w-10 h-10 md:w-12 md:h-12 bg-gradient-brand rounded-xl flex items-center justify-center text-white shadow-brand">
                            <Icon name={item?.icon} size={16} className="md:size-5" />
                          </div>
                        </div>
                        <div className="flex-1 min-w-0">
                          <h3 className="text-lg md:text-xl font-bold text-foreground mb-1">{item?.title}</h3>
                          <div className="text-sm md:text-base text-accent font-medium mb-1">{item?.role}</div>
                          <p className="text-sm md:text-base text-muted-foreground leading-relaxed mb-3 md:mb-4">{item?.description}</p>
                          
                          {activeItem === index && (
                            <div className="animate-slide-up">
                              <h4 className="font-semibold text-foreground mb-2 text-sm md:text-base">Key Achievements:</h4>
                              <ul className="space-y-1 list-none pl-0">
                                {item?.achievements?.map((achievement, idx) => (
                                  <li key={idx} className="flex items-start text-xs md:text-sm text-muted-foreground">
                                    <Icon name="CheckCircle" size={14} className="text-accent mr-2 flex-shrink-0 mt-0.5 hidden md:block" />
                      ...
                      <div className="mt-3 md:mt-4 flex items-center justify-between">
                        <div className="text-xs text-muted-foreground">
                          Tap to {activeItem === index ? 'collapse' : 'expand'} details
                        </div>
                        <Icon 
